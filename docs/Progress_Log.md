...
进入角色恢复等待；客户端 abort 能通过事件立即唤醒并结束等待协程。该测试与零实例
启动窗口取消测试一起纳入统一 CoreX unit gate。

## 2026-09-04：零实例 P/D 等待取消竞态

修复 P/D 服务刚启动或两类 actor 同时重启、Manager 尚未登记任何实例时的取消竞态。
请求现在在进入 `num_instances==0` 重试循环前登记为等待者，abort 可立即移除该 ID；
生成协程每次重试都会检查取消状态，不再因尚无 actor 而永久挂起。新增早期取消回归。
